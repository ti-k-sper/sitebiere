...
$beerArray = [
	...
	for ($i=0; $i < count($beerArray); $i++) {
			echo '<div class="nameBeer"><br/><h1 class="name">'.($beerArray[$i][0]).'</h1><br/><img class="picture" src="'.($beerArray[$i][1]).'" alt="imade la marque de bière"/><br/><p class="desc">'.($beerArray[$i][2]).'</p><br/><p class="price">'.str_replace('.',',',round($beerArray[$i][3]*$TVA,2)).'€</p><br/></div>';
	}
?>
